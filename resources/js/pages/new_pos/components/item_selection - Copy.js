...
function ItemSelection(props) {
    ...
    const updateItems = (value, row) => {
        ...
        if (!isNaN((parseFloat(value) + 0))) {
            ...
            if (resultSel.length > 0) {
                console.log(resultSel[0].qty)
                if ((resultItm[0].balance + 1) > (resultSel[0].qty + parseFloat(value))) {
                // if ((resultItm[0].balance == 1? resultItm[0].balance :(resultItm[0].balance + 1)) >= (resultSel[0].qty + parseFloat(value))) {
                    // setTimeout(function () {
                    newData(value, row)
                    // }, 1000);
                } else {
                    toast.warn("Insufficient Balance f")
        ...
        if (!isNaN((parseFloat(value) + 0))) {
            ...
            if (resultSel.length > 0) {
                if ((resultItm[0].balance + 1) > parseFloat(value)) {
                    // if ( resultItm[0].balance  > resultSel[0].qty) {
                    // setTimeout(function () {
                    updata(value, id)
                    // }, 1000);
                } else {
    ...
    const updata = (value, id) => {
        ...
        var updatedComment = update(selItem_si[commentIndex], { qty: { $set: parseFloat(value) } });
        ...
        if (!isNaN(value) && value > 0) {
            ...
            if (row.balance >= value) {
                ...
                if (exist == "yes") {
                    ...
                    var updatedComment = update(selItem_si[commentIndex], { qty: { $set: parseFloat(result[0].qty) + parseFloat(value) } });
    ...
    function getAmountDue() {
        var amt = 0;
        selItem_si.map((row) => {
            amt += parseFloat(row.qty) * parseFloat(row.unit_price)
        })
                                                            ...
                                                            return (
                                                                <TableRow hover role="checkbox" tabIndex={-1} key={row.code}>
                                                                    ...
                                                                    <TableCell align="right">
                                                                        ...
                                                                        {/* <TextField onChange={(e) => setItmQty_si(e.target.value, row)} style={{ width: 70 }} size="small" label="Qty" variant="outlined" type="number" /> */}
                                                                    </TableCell>
