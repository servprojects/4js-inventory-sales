...
class SupplierLedger extends Component {
  ...
  buttonFormatter = (cell, row) => {
    ...
    return (
      ...
      <div>
        <center>
          <Link to={{ pathname: '/specSupplierLedger', state: { id: row.id, path: '/specSupplierLedger'} }}><button type="button" class="btn btn-primary"  > Ledger </button></Link>
        </center>
      </div>
    )
  }
  render() {
    ...
    var disnotLedger = { display: "block" }
    var hide = false;
    if (this.props.notLedger == "True") {
      disnotLedger = { display: "none" };
      hide = true;
    }
    ...
    return (
      <div className="contentTransact">
        ...
        <h1 style={disnotLedger}>Supplier Ledger</h1>
        <BootstrapTable
          ref='table'
          data={data}
          pagination={true}
        ...
        >
          ...
          <TableHeaderColumn hidden={hide} dataField='id' width="130" isKey={true} dataFormat={this.buttonFormatter} ></TableHeaderColumn>
